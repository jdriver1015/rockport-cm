...
export default async function GlPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [batches, counts, [archivedCount]] = await Promise.all([
    db()
      .select()
        ...
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {archivedCount.count > 0 && (
              <Link
                href={`/properties/${slug}/gl/archived`}
                className="text-sm text-link hover:underline"
              >
                Archived ({archivedCount.count})
              </Link>
            )}
            <AddGlDialog propertyId={property.id} propertySlug={property.slug} />
          </div>
        </CardHeader>
        <CardContent>
          {batches.length === 0 ? (
